...
#Support=Freq of X/T(Transaction), Confidence=Freq(XY)/Freq(X),
...
#provide the details of first 5 transaction
itemFrequency(groceries[,1:3])
#allows us to see the proportion of transaction that contains the item and to view
#the support level for the first three items in the grocery data
...
#combining it with the sample() function, you can view the sparse matrix for a
#randomly sampled set of transactions.
apriori(groceries)
...
#if lift more than 1, it means that the two items are found together more often than by
#chance. Lift of greater than 1 means products A and B and more likely to be bought together.
